<x-app-layout>
    <x-slot name="header" style="background-color: #cfebf8">
    ...
    <div class="mt-4 max-w-7xl mx-auto sm:px-6 lg:px-8">
        <form method="POST" action="{{ route('comment') }}">
            ...
            <textarea 
                name="message" 
                placeholder="{{ __('Berikan pengalaman anda?') }}" 
            ...
            <div class="p-6 flex items-start justify-between space-x-4" >
                ...
                <div class="flex-1">
                    <div class="flex justify-between items-center">
                        <div>
                            <span class="text-gray-800">{{ $rating->user->name }}</span>
                            <small class="ml-2 text-sm text-gray-600">{{ $rating->created_at->format('j M Y, g:i a') }}</small>
                        ...
                        @if ($rating->user->is(auth()->user()))
                            <x-dropdown>
                                ...
                                <x-slot name="content">
                                    <x-dropdown-link :href="route('edit', $rating)">
                                        {{ __('Edit') }}
                                    </x-dropdown-link>
                                    <form method="POST" action="{{ route('hapus', $rating) }}">
                                        @csrf
                                        @method('delete')
                    ...
                    <p class="mt-4 text-lg text-gray-900">{{ $rating->message }}</p>
                </div>
            </div>
            @endforeach
        </div>
